 ...
 */
export async function computeMessageHash(messageSlots, origin, destination) {
...
export function importAccount(eddsa, privKey) {
    return {
        privKey,
 ...
 * @param {Buffer} privKey - The private key used to sign the message 
 * @returns Signature
 */
export function signMessage(eddsa, messageHash, privKey) {
    const signature = eddsa.signPoseidon(privKey, utils.leInt2Buff(messageHash));
 ...
 * @param {bigint} options.R8y - The R8y parameter from the signature
 * 
 ...
 * @param {bigint} options.publicInputs.witnessAddr - The address of the witness
 ...
 */
export async function computeProof({ Ax, Ay, S, R8x, R8y, publicInputs, snarkArtifacts }) {
    const input = {
        //Private inputs
        Ax,
        Ay,
